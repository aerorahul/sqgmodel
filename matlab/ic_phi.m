...
%
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
% SPECIFY geopotential (phi) at the tropopause.
% OBTAIN, potential temperature (theta)
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
switch(Nic)
	case{1}
  	disp(' Gaussian Formulation')
		...
		phiT_xy = fxy;             % spaatial phi   at Tropopause
		phiT_sp = fft2(phiT_xy);   % spectral phi   at Tropopause
		thetaT  = DZ.*phiT_sp;     % spectral theta at Tropopause
  	
		phiB_xy = zeros(Ny,Nx);    % spaatial phi   at Ground
		phiB_sp = fft2(phiB_xy);   % spectral phi   at Ground
