...
SUBROUTINE pvinv_main()
    ...
    real, dimension(2*kmax,2*lmax)      :: thbB, thbT
    real, dimension(2*kmax,2*lmax,pmax) :: pvb
...
! Solve for psi(z) given pv(z) for a given (k,l) in ** uniform ** N^2:
    ...
    complex, dimension(:), intent(in)  :: pv
    real,                  intent(in)  :: ak,bl
    integer,               intent(in)  :: idn
    complex, dimension(:), intent(out) :: psi
...
! recover winds, theta, and geopotential from potential data.
    ...
    complex, dimension(2*kmax,2*lmax,pmax), intent(in)  :: phi0,phi1
    complex, dimension(2*kmax,2*lmax,pmax), intent(in)  :: F1,G1,P1
    complex, dimension(2*kmax,2*lmax),      intent(in)  :: phi0b,phi0t
    complex, dimension(2*kmax,2*lmax),      intent(in)  :: tb,tt
    complex, dimension(2*kmax,2*lmax,pmax), intent(out) :: u,v,w
    complex, dimension(2*kmax,2*lmax,pmax), intent(out) :: thF
...
! bpvsp : boundary condition trick (make homogeneous BCs; correct int).
    ...
    complex, dimension(2*kmax,2*lmax),      intent(in)  :: tbsp,ttsp,phi0b,phi0t
    complex, dimension(2*kmax,2*lmax,pmax), intent(in)  :: phi0,pvsp
    complex, dimension(2*kmax,2*lmax),      intent(out) :: phi1b,phi1t
    complex, dimension(2*kmax,2*lmax,pmax), intent(out) :: F1,G1,P1,phi1
...
SUBROUTINE w_check(u,v,win,wout)
    ...
    do p = 1, pmax
        ...
        wnew = wold - (ux + vy)*dz
        !print*,'max ux = ',maxval(abs(ux)),maxval(abs(vy)),dz, maxval(abs(ux+vy))
        !wF = (wnew + wold)/2. !why???
        wF = wnew
        wout(:,:,p) = wnew
        wold = wnew; wxn = 0.
        ...
        if (verbose .gt. 1) print*,'max w comparison (old,new): ',maxval(abs(wxo)), maxval(abs(wxn))
    enddo
...
! setup Jacobian factors for tanh vertical grid
    ...
    do k = 1,pmax     
        ...
        if ( verbose .gt. 1 ) print*,'jacobian:',k,z,zl(k),facz(k)
    enddo
    ...
    do k = 1,pmax-1
        ...
        if ( verbose .gt. 1 ) print*,'jacobian:',k,z,zhl(k),faczo(k)
    enddo
...
!========================================================================
SUBROUTINE epv_pseudoheight(u,v,t,ub,vb,tb,ut,vt,tt,epv_out)
...
! all variables are SPECTRAL and NON-DIMENSIONAL, _except_ for out.!        
    ...
    complex, dimension(2*kmax,2*lmax,pmax), intent(inout) :: u,v,t
    complex, dimension(2*kmax,2*lmax),      intent(in)    :: ub,vb,tb,ut,vt,tt
    real,    dimension(2*kmax,2*lmax,pmax), intent(out)   :: epv_out

    complex, dimension(2*kmax,2*lmax,pmax) :: uz,vz,tz
    complex, dimension(2*kmax,2*lmax)      :: dx,dy,Id,vx,uy,tx,ty
    complex, dimension(2*kmax,2*lmax)      :: tmp
    ...
    do k=1,pmax
        ...
        tmp = vx - uy + tz(:,:,k) + Ross*( (vx - uy)*tz(:,:,k) - vz(:,:,k)*tx + uz(:,:,k)*ty )
        ! map back to physical space
        call sp_to_xy(tmp,epv_out(:,:,k),kmax,lmax,2*kmax,2*lmax)
        !call sp_to_xy(ty(:,:),out(:,:,k),kmax,lmax,2*kmax,2*lmax)
        if (verbose .gt. 1) print*,'max recovered pv at level ',k,' : ',maxval(abs(epv_out(:,:,k)))
...
!========================================================================
SUBROUTINE epv_pseudoheight_new(u,v,t,tb,tt,phi0,phi0b,phi0t,epv_out)
! inputs: horizontal winds on grid levels: u,v.
!         horizontal winds on grid boundaries: ub,vb.
!         potential temperature on grid levels: t
!         potential temperature on grid boundaries: tb
...
! all variables are SPECTRAL and NON-DIMENSIONAL, _except_ for out.!        
    ...
    complex, dimension(2*kmax,2*lmax,pmax), intent(inout) :: u,v,t,phi0
    complex, dimension(2*kmax,2*lmax),      intent(inout) :: tb,tt,phi0b,phi0t
    real, dimension(2*kmax,2*lmax,pmax),    intent(out)   :: epv_out
    !
    complex, dimension(2*kmax,2*lmax,pmax) :: uz,vz,tz,u0,v0
    ...
    do k=1,pmax
        ...
        ! map back to physical space
        call sp_to_xy(tmp,epv_out(:,:,k),kmax,lmax,2*kmax,2*lmax)
        !call sp_to_xy(vx-uy,out(:,:,k),kmax,lmax,2*kmax,2*lmax)
        !call sp_to_xy(tz(:,:,k),out(:,:,k),kmax,lmax,2*kmax,2*lmax)
        print*,'--max recovered pv at level ',k,' : ',maxval(abs(epv_out(:,:,k)))
